...
function listenForClicks() {
  document.addEventListener("click", (e) => {
    ...
    function beastify(tabs) {
      ...
      chrome.scripting.insertCSS({css: hidePage, target: {tabId: tabs[0].id}}).then(() => {
    ...
    function reset(tabs) {
      // V2 → V3 chrome.tabs.insertCSS → chrome.scripting.insertCSS
      // required: `target`, `code` → `css`
      chrome.scripting.removeCSS({css: hidePage, target: {tabId: tabs[0].id}}).then(() => {
        chrome.tabs.sendMessage(tabs[0].id, {
          command: "reset",
        });
      });
...
chrome.tabs.query({active: true, currentWindow: true}).then(async ([{id: tabId}]) => {
  // V2 → V3: chrome.tabs → chrome.scripting
  // required: `target`, `file` → `files`
  await chrome.scripting.executeScript({files: ["content_scripts/beastify.js"], target: {tabId}})
  listenForClicks()
